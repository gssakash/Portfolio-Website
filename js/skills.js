kindChart($(".chart.skills"), {
  ...
  datasets: [
    // 0 being farthest back, 1 being closest
    {
      ...
      fillColor: "#ef0d33",
      strokeColor: "#ef0d33",
      data: [90, 100, 100, 100, 80, 80, 90, 80]
    }
  ]
});
...
function kindChart(element, options) {
  ...
  function getColor(passed) {
    if (passed == undefined) {
      return (
        "rgba(" + colors[Math.floor(Math.random() * colors.length + 0)] + ",1)"
      );
    } else {
      return passed;
    }
  ...
  for (var i = 0; i < options.datasets.length; i++) {
    ...
    var webLine = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "path"
    );
    var webstream = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "path"
    );
    var path = "";
    ...
    element
      ...
      .append(
        '<li style="color: ' +
          getColor(options.datasets[i].strokeColor) +
          ';">' +
          options.datasets[i].label +
          "</li>"
      );
    ...
    for (var x = 0; x < options.datasets[i].data.length; x++) {
      ...
      var point = findNewPoint(
        (360 / options.datasets[i].data.length) * x,
        options.datasets[i].data[x]
      );

      var web_points = findNewPoint(
        (360 / options.datasets[i].data.length) * x,
        100
      );
      ...
      for (var l = 0; l < 5; l++) {
        ...
        if (x == 0) {
          ...
          //console.log(x);
          websteam_lines[l] +=
            " L" + stream_point.x + " " + stream_point.y + webstream_ends[l];
        } else {
    ...
    select_circle[i] = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "circle"
    );
  ...
  for (var x = 0; x < options.datasets[0].data.length; x++) {
    ...
    var text = findNewPoint(
      (360 / options.datasets[0].data.length) * x,
      113 + ((360 / options.datasets[0].data.length) * x) / 90
    );
  ...
  element.mousemove(function (e) {
    if (go == true) {
      ...
      var angle =
        Math.atan2(e.pageX - objCenterX, -(e.pageY - objCenterY)) *
        (180 / Math.PI);
      ...
      for (var i = 0; i < options.datasets.length; i++) {
        outtext +=
          '<li style="color: ' +
          options.datasets[i].strokeColor +
          ';">' +
          options.datasets[i].label +
          ": " +
          options.datasets[i].data[out] +
          "</li>";

        var pointl = findNewPoint(
          (360 / options.datasets[i].data.length) * out,
          options.datasets[i].data[out] + 1.75
        );
      ...
      $(this)
        .find(".out ul")
        .html("<p>" + options.labels[out] + "</p>" + outtext);
    }
  });
